...
| markdown-renderer-014 | support-extended-attributes | (Apple) `"^[Attributed](customKey: \"value\")"` | Parses successfully via `allowsExtendedAttributes: true` (no fallback to plaintext); rendered text is "Attributed" — the `customKey` value is not applied to `.font`/`.foregroundColor` because the renderer does not read arbitrary attribute keys |
