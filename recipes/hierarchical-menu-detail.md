...
- **pin-toggle**: Each rendered list's header MUST carry a `«`/`»` toggle that pins the list open or closed, independent of `autoHide`, persisting across selections. The toggle is always present in `covered` and `cascading` styles; in `minimized` style it is present only when `manualCollapse` is true (default). A pin overrides `autoHide`: pinned-hidden (`true`) MUST keep the list hidden; pinned-disclosed (`false`) MUST keep it disclosed unless width pressure forbids it. Flipping `autoHide` MUST clear every pin.
...
- **narrow-mode**: When `layoutMode` is "narrow", or when "auto" and the container is narrower than one topic list plus `minDetailWidth`, or when the browser reports a phone user agent (checked after mount, via a user-agent pattern for iPhone/iPod or Android + Mobile), component MUST render one full-width pane at a time (the frontier list, or the detail once every level is selected), with a Back button — shown for every disclosure style — to navigate up the hierarchy by clearing the deepest selected level.
