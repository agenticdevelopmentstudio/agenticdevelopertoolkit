...
interface TopicLevel {
  ...
  onSelect: (id: string, options?: { replace?: boolean }) => void   // select THIS level (clears descendants);
                                   // options.replace avoids pushing history (used by an auto-applied
                                   // default — see default-select-a-level). Pure nav: push(`…/<id>`)
  ...
  headerSlot?: ReactNode           // pinned non-scrolling strip between the title header and the rows
                                   // (hosts the shared ListHeader for entity-list levels in the stack)
}
```

Each level's `items` are `TopicDetailItem`s:

```ts
interface TopicDetailItem {
  id: string
  label: string
  ...
  preview?: string
  previewLines?: number
  description?: string             // no surface in THIS view renders it (the retired overview grid did) —
                                    // see describe-topics
  icon?: ReactNode
  leadsTo?: "list" | "detail"       // what a row discloses (the cascade's declared leafness)
...
at the clicked list itself (`root-reveal-on-covering-select`): the clicked list stays open in
...
4. **Then narrow mode.** When they are all hidden, the HTDV is in **narrow mode** — it now
   ...
   nothing left to trade (**exhaust-wide-before-narrow**), never earlier.
...
- **The flip carries the detail with it, state intact.** The detail's subtree is never remounted by a
  ...
  view in narrow mode (see keep-selection-across-modes).
...
| Disclosure | agenticdevelopertoolkit://recipes/disclosure | The animated disclosure (collapse/expand) of a topic list to/from its icon strip. | yes | Upper-right toggle in the `minimized` style; the default `covered` style rides the child's upper-left leading slot instead. Animated unless reduced-motion. |
...
> **Reference consumer.** Examples throughout this recipe are illustrated against this toolkit's own ADH hub integration — Work Items, the status board, `WorkspaceShell`, `ResourceTab` — to keep every requirement concretely testable. These names identify one consumer, not the contract: substitute `{{app_domain}}`, `{{workspace_slug}}` and your own feature/entity names when implementing this recipe elsewhere.

## Integration Requirements

### Hierarchy & deep linking

- **nest-topic-lists**: The view MUST render an ordered stack of topic lists where each list's selection scopes the next; the deepest open level's pane renders the leaf detail.
- **deep-link-every-level**: Every level of the hierarchy MUST be addressable by a URL path segment, so the full selection is shareable and restorable — e.g. `https://{{app_domain}}/{{workspace_slug}}/ecosystems/temporal/applications/notes`.
- **restore-from-url**: On load (and on browser back/forward), the view MUST restore each topic list's selection and the open leaf from the URL path.
- **update-url-on-select**: Selecting an item in any topic list MUST update the URL to that level's path; deeper segments are cleared unless the next level names a `defaultSelectedId`, in which case that default's own selection applies (see **default-select-a-level**) — never a blanket "reset to a default", which would contradict **not-auto-select**.

### Top chrome — breadcrumb + help

- **render-one-breadcrumb-bar**: The view MUST render exactly ONE breadcrumb bar across the full width of the top, spanning all topic lists and the detail pane (not one per rail).
- **show-full-trail**: The breadcrumb MUST show the selected item of every topic list in order (workspace ▸ feature ▸ entity ▸ topic ▸ …); the last crumb is the current location (`aria-current`).
- **navigate-from-crumb**: Clicking an ancestor crumb MUST navigate to that level, deselecting everything deeper (via that level's `onClear`, gated by the unsaved-work guard).
- **render-breadcrumb-help**: The breadcrumb bar MUST carry a right-justified help button that pops up a description of the current view (sourced from the help config).

### Alignment

- **align-list-and-pane-top-edges**: The top edges of every topic list and the detail pane MUST be vertically aligned on one row (directly under the breadcrumb).
- **align-list-and-pane-bottom-edges**: Every topic list and the detail pane MUST fill to the same height, so their bottom edges are vertically aligned.
...
- **left-justify-content**: Content in topic lists and detail panes MUST be left-justified. Centered content is a special-case exception (e.g. an empty-state prompt), not the default.
- **expand-detail-horizontally**: A detail pane MUST expand to fill the available horizontal space, honouring a minimum width.
- **scroll-horizontally-below-min**: When the available space is narrower than the detail pane's minimum width, the pane MUST scroll horizontally rather than crush its content.
- **fill-detail-vertically**: A detail pane MUST fill the available vertical space of its container.
- **avoid-fixed-vertical-scroll**: A detail pane MUST NOT contain fixed-size content that forces a vertical scrollbar where the content could instead grow to fit; vertical scrolling is a last resort.
- **render-detail-action-bar**: A leaf editor detail MUST render its action bar (Save / Cancel / Delete, and the package-injected Back when drilled) as the top of the leaf pane (not hoisted to the top chrome). The full-width breadcrumb names the pane, so a separate centered title is redundant.
- **render-detail-help-icon**: Help for the current view is the right-justified icon on the breadcrumb bar (sourced from the help config); a leaf editor MAY additionally carry a topic-specific help icon on its action bar.

### Per-view layout — topic lists

- **render-icon-name-rows**: Each topic row MUST render as `[icon] [name]`.
- **fix-list-width**: A topic list MUST be a fixed width — slightly wider than its widest topic — with consistent leading and trailing padding.
- **offer-new-topic-button**: A topic list MAY carry a "new topic" (`onNew`) create affordance, rendered as a compact **`+` button right-justified in the list header** (NOT a leading row); activating it MUST open the create MODAL (see **create-in-modal**). The `+` carries its `newLabel` as its accessible name + tooltip, and MAY tint gold (`newActive`) while the modal is open.
- **offer-title-actions**: A topic list MAY carry extra compact controls right-justified in its TITLE row, just ahead of the `+` (`titleActions` — e.g. the status board's Sites list hosts its Auto Configure action there, freeing the `headerSlot` filter field to take the full row width via `ListHeader`'s `search.grow`). Title-row actions render only with a titled, un-collapsed header; keep them one line high.
...
- **scope-create-modal-to-placement**: The create modal MUST ask only for what brings the record into existence and PLACES it (a name/title, and the parent/status/column it lands in) — not the record's full editor. Everything else belongs to the detail that opens on save. (New Project = name + description; New Work Item = title, description, status.)
- **delete-row**: A topic row MAY declare `onDelete` to expose a **right-justified trash button, revealed only on hover (and keyboard focus)**; the list's fixed width MUST account for it (the row reserves trailing space so the label never runs under the button). It is NOT shown on the collapsed / covered icon strips.
- **confirm-row-delete**: Activating a row's trash button MUST open a confirmation before anything is destroyed — a destructive [[alert-and-dialog]] modal (red action, keyboard shortcuts off, initial focus on Cancel). `onDelete` runs ONLY on confirm; it MAY be async, and the dialog MUST show a busy spinner (and block dismissal) until it settles.
- **break-connector-around-delete**: In the stack, the selection connector line MUST break (leave a gap) around a selected parent row's trash button rather than crossing it — the overlay paints above the rail, so the break is a computed gap in the path, not occlusion.
- **offer-disclosure-toggle**: A topic list MUST offer a way to disclose/undisclose it. In the `minimized` style each list's own toggle rides its header's upper right, alongside `titleActions`. In the default `covered` style the toggle instead rides the CHILD's header upper-left (leading control slot), covering/uncovering that child's parent — see the Layout section. Disclosing/undisclosing MUST be animated (subject to **respect-reduced-motion**).
- **render-undisclosed-icon-strip**: An undisclosed topic list MUST render as a vertical list of the topics' icons; the header `+` collapses with the header (reachable via the whole-branch reveal when covered).
- **fill-list-vertically**: A topic list MUST fill the available vertical space (pinned to its container's height) and MUST scroll only when its items overflow.
- **resize-by-drag**: A topic list MUST be horizontally resizable by dragging its trailing border.
- **snap-undisclosed-when-narrow**: If dragged narrower than 1/3 of its full width, a topic list MUST animate to undisclosed.
- **snap-full-when-wide**: If dragged wider than its content plus the default padding, a topic list MUST animate (back) to its full content width.

### Selection — pure intent, package-owned

- **split-select-clear**: A level MUST expose `onSelect(id)` (select this level, clear descendants) and `onClear()` (clear this level + everything below) as PURE navigation; the package decides WHEN each fires.
- **own-unselection**: A click on the already-selected row MUST clear that level (`onClear`); a click on any other row MUST select it (`onSelect`). Consumers MUST NOT write toggle logic.
- **not-auto-select**: The view MUST NOT auto-select anything of its own accord — landing on a level with no selection shows the list with nothing focused (no resume of a last id, no coerced first item); only the deepest pane that IS selected renders a detail. The ONE exception is a level that explicitly asks for a landing selection (**default-select-a-level**); a level that names none is never chosen for.
- **default-select-a-level**: A level MAY name a `defaultSelectedId` — the item to select the moment that list APPEARS with nothing chosen (i.e. when the parent topic that opens it is picked). It MUST arrive as an ordinary selection, fired through the level's own `onSelect`, so the URL, the breadcrumb, the detail and re-click-to-clear all behave exactly as if the user had clicked the row — with ONE exception, **not-spend-history-on-a-default** below. It MUST arm once per APPEARANCE: clearing the row INSIDE that visit MUST stick (a default that re-fires on every clear makes the row impossible to deselect — a default may choose FOR the user, never argue WITH them), while leaving the parent topic and returning MUST re-apply it. **A list that APPEARS already showing the selection the default names counts as an armed appearance** — the visit is spent whether the default fired or was already satisfied, or a deep link straight to the defaulted item leaves nothing recorded and the first clear reads as a fresh appearance with nothing chosen, so the default re-fires and the row cannot be deselected at all. A default naming an item the list does not (yet) have MUST simply not fire — an async list applies it when its rows land, and a stale default MUST NOT select a phantom row. Use it where the answer is all but certain and an empty pane would just ask a question with one sensible answer (Work Items → List); the default for every other level remains **not-auto-select**.
- **not-spend-history-on-a-default**: An auto-applied default MUST NOT cost a history entry. A choice the package made is not a place the user navigated to, so the default MUST REPLACE the current entry where a click would PUSH — the level's `onSelect` therefore takes an optional `{ replace: true }`, passed on the default path and omitted on a click. Otherwise one click on the parent topic yields TWO entries (the topic, then the default's leaf) and Back lands on the bare topic, where the default immediately re-applies and throws the user forward again — a Back button that visibly does nothing. The bare topic stays reachable by CLEARING the row (**default-select-a-level**), which is a state the user chose; it is not a Back stop. A level whose `onSelect` ignores the option reintroduces the double entry, so a level that names a `defaultSelectedId` MUST honour it.
- **be-one-stack**: Every NAVIGATION list MUST be a level of the single stack — a list is "navigation" when picking a row is how you REACH a record (the row becomes the selection, the URL grows a segment, and the pane beside it becomes that record's detail). An in-pane master/detail of that kind MUST be dismantled into a published list level + a leaf editor. There MUST be exactly one navigator.
- **display-content-as-a-list**: A detail pane holds CONTENT, and content MAY render as a list — a list is one of the ordinary shapes data comes in, beside a board, a table, a timeline, a calendar or a chart. A display MAY carry its own details pane and MAY edit its rows in place ([[list-with-details-pane]] + [[inline-commit-control]]), because none of that is navigation: the stack does not move, no level is selected, and the URL does not grow. "The deepest pane is a detail" constrains the pane's ROLE, not its shape.

### Whole hierarchy — disclosure + off-screen drill-down

- **single-disclosure-controller**: ONE ResizeObserver over the whole row MUST drive disclosure/drill-down for every list (not one observer per list).
- **keep-manual-toggle**: Each list MUST keep its manual disclosure toggle (the `«` that shrinks it to a vertical icon strip); a manually-collapsed list counts as its icon width in the fit math, then slides off-screen if there is still no room.
- **undisclose-before-off-screen**: Shrinking the window is a TWO-PHASE response. Phase 1 — as the window shrinks, the **leftmost** still-full lists MUST first **undisclose** to their icon strips (general → specific) until everything fits or every list is an icon strip. Phase 2 — ONLY once every list is already an icon strip AND those icon strips plus the detail's minimum STILL don't fit may lists begin to slide off-screen. A list MUST NOT slide off-screen while it (or a list to its right) could still be undisclosed instead.
- **drill-off-screen-when-cramped**: In phase 2, the **leftmost** icon-strip lists MUST slide **off the left edge** (hidden: `0`-width column + `inert` + `aria-hidden`), general → specific, until the detail fits at its minimum. On a phone width every list slides off (detail full-width), computed before paint so the first frame is already drilled-down.
- **not-hide-frontier-choosing-list**: The view MUST NOT hide the frontier list while it has no selection (its "detail" is only a landing) — only once that level is selected (the detail is real content) may the frontier list also slide off.
- **disclose-when-room**: When the window widens, hidden lists MUST slide back on (specific → general) as space allows.
- **show-back-when-drilled**: When at least one list is hidden AND something is selected, a **Back** button MUST appear top-left of the leftmost-visible pane (the detail's button bar if the detail is leftmost-visible, else the top-left of the leftmost-visible list).
- **back-clears-one-level**: Back MUST clear exactly the deepest selected level (`onClear`), re-disclosing one parent; repeated Back walks up to root.
...
- **not-redisclose-user-collapsed**: Automatic disclosure MUST NOT re-disclose a list that the user explicitly undisclosed (user intent wins over the layout heuristic).
- **keep-detail-at-minimum-while-lists-yield**: Shrinking MUST take the room from the LISTS, never from the detail: the detail MUST hold `minDetailWidth` through both phases, and MUST only be narrower than it once every list is off-screen and it is the sole view — where it MUST be exactly the container's width.
- **shift-off-screen-by-whole-lists**: In phase 2 the off-screen shift MUST be quantised to WHOLE lists — shift the stack left by exactly the leftmost visible list's width, hiding that list, and repeat. A partial shift (parking a list half off the left edge) is a defect.
- **hide-off-screen-progressively** (v1.17.0): Phase 2 MUST play out ONE list per strip-width of shrink — each further strip-width of narrowing hides exactly the next leftmost strip — and the wide→narrow mode switch MUST NOT preempt it (see **exhaust-wide-before-narrow**). All the collapsed lists disappearing at once as the window shrinks is a defect (the shipped behavior this rule replaces: the old narrow threshold, one full list + the detail minimum, was ABOVE the width where phase 2 begins, so the mode flip swallowed the whole progression).
- **reverse-on-grow**: Widening MUST re-run the same rules on the wider container, so on a larger device leaving narrow mode discloses the lists in reverse order — off-screen lists slide back on (specific → general) and hidden lists re-disclose — but a list MUST re-disclose ONLY when `autoHideTopics` is off and the user has not pinned it shut.
- **log-layout-decisions-outside-production** (v1.17.0): The layout engine MUST trace its decisions to the console — the wide↔narrow mode decision (with the width and floor it was made at), each fit pass's collapse / off-screen outcome, every disclosure toggle, and every narrow push/pop — on EVERY environment except production, where it MUST be off. The switch is host-flipped (`setHtdvLayoutLog`), sitting behind the host's build-inlined environment allowlist so a production bundle ships it dead-code-eliminated; the lines are also kept in a bounded buffer (`__htdvLogDump()`).

### Auto-hide — lead with the leaf

- **default-to-auto-hide**: The frame MUST default to `autoHideTopics = true`: only the LEAF-MOST topic list is disclosed and every parent is hidden by its child even when there IS room for it. A surface whose ancestry is its navigation (the hub's workspace `/home`) MUST opt out with `autoHideTopics={false}`.
...
- **draw-one-chain-line**: The selection chain MUST read as ONE line wherever it is drawn. Three
  ...
  number of pixels.
- **bounce-the-entrance**: A submenu disclosed by choosing a row MUST grow out of that row and
...
- **not-wiggle-the-exit**: The exit MUST NOT wiggle — it shrinks straight back into the row that
...
- **animate-every-menu-closure**: EVERY click that tears menus down MUST collapse them
  ...
  swallow the click.
- **collapse-inward**: When a parent is unselected/replaced, its descendant menus MUST collapse
...
- **hold-the-ground-under-the-pointer**: The GROUND — the root list's right edge, which is also
  ...
  (collapse-from-one-pointer-authority). The frozen value lives in the surface store and so
  survives the remount a selection causes (**keep-view-state-across-a-selection**).
- **collapse-from-one-pointer-authority**: The cascade's disclosure MUST be a STORED two-state
  ...
  auto-collapse-menus-on-final-choice). A rail click is NOT a settling event and MUST stay
...
- **hold-the-detail-until-the-final-choice** (v1.15.0, rebuilt v1.16.0 on DECLARED leafness):
  ...
  cascade this overrides show-select-nudge-at-unselected-frontier while the user is choosing
  ...
  pure-select-from-reveal for intermediate selects. The hold is a DISPLAY hold, not a deferred
  navigation: the intermediate select still fires the level's `onSelect`, still clears the deeper
  selections and still moves the URL — so guard-unsaved-on-exit still runs on the select that
  clears a dirty leaf, and the held content MUST survive the remount that select causes
  (keep-view-state-across-a-selection: held per surface, exactly like the ground).
  ...
  release-the-hold-when-the-gesture-ends.
- **release-the-hold-when-the-gesture-ends** (v1.16.0, T62): The detail hold ends with the
...
- **auto-collapse-menus-on-final-choice** (v1.15.0): When the final choice is made in auto-collapse mode (`autoHideTopics` on),
  the menus MUST auto-collapse on the click itself: the open submenus collapse progressively
  (animate-every-menu-closure, collapse-inward) as the final choice's detail shows, WITHOUT
  waiting for the pointer to leave the menu region. This is the ONE click that closes the menus — the
  carve-out named in collapse-from-one-pointer-authority — and it closes them for the same
  reason the disclosure toggles settle the stack (apply-disclosure-toggles-immediately): the
...
- **not-move-the-menus-on-an-intermediate-select** (v1.15.1, rebuilt v1.16.0): An INTERMEDIATE
  ...
  bounce-the-entrance). The stay-open MUST be STRUCTURAL, not a pointer-reveal side effect.
  ...
  frozen base (collapse-from-one-pointer-authority): a rail click ENGAGES at the clicked list
  ...
  (not-expand-parents-on-select) — while every already-on-screen column keeps painting from
...
- **draw-every-detection-frame**: With the "Show Mouse Detection Frames" debug switch on, EVERY
  ...
  separate "collapse" and "ground held" rects — the unification of collapse-from-one-pointer
  -authority made visible.)

### General

- **respect-reduced-motion**: All animations (disclosure, resize snap, auto-disclosure, the narrow push/pop) MUST be disabled when the user's "reduce animation" preference is set. **SUSPENDED** for both hierarchical views by a standing instruction from the block's owner to ignore `prefers-reduced-motion` until further notice — HTDV's `motion-reduce:` gates were removed so the two views cannot disagree once one flag picks between them. Restore both together when the instruction is lifted. (This is not a claim about anyone's OS configuration.)
- **source-help-from-config**: All help content (the breadcrumb help button and every detail-pane help icon) MUST come from a single unified help config owned by the consuming application, keyed by the route to the detail page + the ui element.
...
└─────────┴──────────┴──────────┴──────────┴─┴────────────────────────────┴─┘
  ...
  (narrower → leftmost lists COVER/slide OFF-SCREEN; hover a peek to open every on-screen list — parents and children alike)
```
...
- **Detail pane**: `bg-apt-surface`, flexible width with a minimum (horizontal scroll
  below it), fills height; its leaf's action bar (Save/Cancel/Delete, and Back when drilled) renders at the top of the
  pane — never a separate centered title, since the full-width breadcrumb already names the pane — with a
  right-justified help icon. Colour only via `apt-*` tokens; no raw hex; no `!important`.
...
|---|---|---|---|
| T1 | nest-topic-lists, align-first-row | render 4 levels, deepest selected | 4 rails + a detail pane; first rows aligned; deepest pane renders the leaf |
| T2 | deep-link-every-level, restore-from-url | load `/{{workspace_slug}}/ecosystems/temporal/applications/notes` | each rail selects its segment; the `notes` leaf opens |
| T3 | update-url-on-select | click entity "temporal" (its child level names no `defaultSelectedId`) | URL becomes `/…/ecosystems/temporal` with the deeper segments cleared — not auto-filled with a default topic |
| T4 | render-one-breadcrumb-bar, show-full-trail | entity+topic selected | one breadcrumb `Acme › Ecosystems › Temporal › Applications`; last is `aria-current` |
| T5 | navigate-from-crumb | click the "Ecosystems" crumb | navigates to the feature, deeper selection cleared |
| T6 | render-breadcrumb-help, source-help-from-config | click the breadcrumb help button | popover shows the config text for this route |
| T7 | align-list-and-pane-top-edges, align-list-and-pane-bottom-edges | viewport 1440 | all rail + pane top edges share a row; all bottom edges share a row |
| T8 | render-detail-action-bar, render-detail-help-icon | open a leaf editor | the action bar (Save/Cancel/Delete) is the top of the leaf; the breadcrumb help icon opens the view's description |
| T9 | expand-detail-horizontally, scroll-horizontally-below-min | shrink viewport below detail min-width | detail keeps min-width and scrolls horizontally (content not crushed) |
| T10 | render-icon-name-rows, fix-list-width | render a rail | rows are `[icon] [name]`; fixed width ≈ widest topic + padding |
| T11 | offer-new-topic-button, create-in-modal | click a rail header's `+` | a MODAL opens (the `+`'s accessible name is `newLabel`); the pane behind it still shows what was open and no level's selection changed; on save the new item is selected and ITS detail opens; on cancel nothing changed |
| T11a | create-in-modal | open the create modal, type into it, press Esc / click the backdrop | the draft is not discarded — the guarded close prompts Save / Discard / keep editing |
| T11b | scope-create-modal-to-placement | open "New Work Item" | it asks for title + description + status (what PLACES the record), not the full item editor; assignee/priority/dates/parent live in the detail that opens on save |
| T12 | offer-disclosure-toggle, render-undisclosed-icon-strip | click a rail's `«` | rail animates to an icon strip; the header (with its `+`) collapses; active icon keeps the marker |
| T13 | resize-by-drag, snap-undisclosed-when-narrow | drag a rail below 1/3 width | rail animates to undisclosed |
| T14 | snap-full-when-wide | drag a rail wider than content+padding | rail animates back to full content width |
| T15 | drill-off-screen-when-cramped, single-disclosure-controller | narrow the window past the detail min-width with a leaf selected | leftmost lists slide off-screen (general→specific) until the detail fits at min |
| T16 | disclose-when-room | widen the window | hidden lists slide back on (specific→general) as space allows |
| T17 | not-redisclose-user-collapsed | user collapses list 2, then widen window | list 2 stays icon-collapsed; others may re-disclose |
| T18 | respect-reduced-motion | `prefers-reduced-motion: reduce` | **SUSPENDED** — see Design Decision **Reduced motion is suspended for this component**; re-enable this vector when the gates return |
| T19 | own-unselection | click the already-selected entity/topic/list row | that level clears + every deeper pane hides (no consumer toggle logic) |
| T20 | not-auto-select | land on `/<slug>/ecosystems` | the entity list shows nothing selected; selecting one shows the topics list with nothing selected (no resume, no first-topic) |
| T20a | default-select-a-level | a level sets `defaultSelectedId` (Work Items → `list`); pick its parent topic | the list appears with that row selected, via the level's own `onSelect` (URL segment, breadcrumb + detail all follow); re-clicking the row clears it and it STAYS clear; leaving the parent topic and re-entering selects it again |
| T20b | not-spend-history-on-a-default | pick the parent topic of a level with a `defaultSelectedId`, then press Back once | ONE press leaves the parent topic entirely (the default's leaf replaced the topic's entry rather than stacking on it) — Back does not land on the bare topic only for the default to bounce you forward again |
| T20c | default-select-a-level | DEEP-LINK straight to the defaulted item's URL (the list appears already showing it), then clear the row | it STAYS cleared — an appearance that arrives already satisfied still spends the visit, so the clear is not read as a fresh appearance with nothing chosen |
| T21 | show-back-when-drilled, back-clears-one-level | drill in at phone width, press Back | Back appears top-left of the leftmost-visible pane; pressing it clears the deepest selected level + re-discloses one parent |
| T22 | guard-unsaved-on-exit | edit the leaf editor (dirty), then press Back | a Save / Discard / Cancel modal opens; Discard proceeds, Cancel keeps editing, Save persists then proceeds |
| T23 | not-hide-frontier-choosing-list | narrow `/<slug>/home` (workspace selected, no feature) | the features list (the frontier) stays visible; only the workspaces list may slide off |
| T24 | be-one-stack | open a dismantled topic (Applications) | the apps are a published list level (not an in-pane sublist); the editor is the leaf detail |
| T24a | display-content-as-a-list | open Work Items ▸ List (a list-with-details display) | the rows render, a row selection fills the details pane and edits in place — and the STACK does not move: no level's selection changed, no new rail appeared, the URL is unchanged |
| T25 | default-to-covered, peek-covered-parent | render ≥3 covered levels, deepest selected | parents peek as 32px icon strips under the child; child + detail take the room |
| T26 | reveal-all-covered-lists-on-hover, open-branch-in-place | hover a covered list's peek (3 levels, both parents covered) | the hovered list AND every list below it open to full width, chained end to end from where the hovered list sat (lefts `0 / 240 / 480`), lifted above the peeks behind it — the detail's left edge slides right to stay visible beside the reveal (push-detail-aside-on-reveal) |
| T26a | keep-branch-open-while-inside-it | hover the covered root, then move the pointer into its revealed CHILD | the branch stays open (this is the walk the reveal exists for); moving the pointer out of every member closes it back to exactly the peeks it came from |
| T26b | keep-branch-open-while-inside-it | hover the covered root, then move the pointer into a DIFFERENT covered list | the branch re-roots there: the new list + its children open, the lists to its left return to their peeks |
| T26c | float-branch-as-an-opaque-card | hover a covered list in the MIDDLE of the stack (a peek to its left, the detail to its right) | the open branch shadows on BOTH outer edges — leading (over the peek behind it) and trailing (over the detail) — and no detail text shows through it; the members between them carry neither shadow |
| T26d | draw-connectors-over-the-revealed-branch | hover a covered list with a selection chain through it | the gold connectors remain visible ON TOP of the revealed lists (the overlay outranks the branch's lift), not hidden behind them |
| T27 | pure-select-from-reveal | click a row in a revealed list | that row becomes selected, deeper lists clear, its detail shows; clicking the already-selected row does nothing |
| T28 | close-branch-only-on-pointer-exit | hover a covered list to reveal the branch, click a row, then walk right into the child list it re-populated and click one of ITS rows | both selections land while the branch stays open under the pointer (the child list joins the open branch); the branch collapses only once the pointer leaves it, animating to the layout the new selection implies |
| T28a | root-reveal-on-covering-select | auto-hide ON, one disclosed list, click a row whose selection pushes a new (unselected) child list | the clicked list stays open in place (covered in layout, held by the reveal) and the new list slides out beside it OVER the detail; moving the pointer off the branch settles it — the parent to its 32px peek, the new list into the flow |
| T28b | root-reveal-on-covering-select | auto-hide ON, click a row in the LAST level (the selection completes the path) | no reveal engages: the stack lays out at rest (parents peek, the clicked list disclosed) with no lift and no floating-card shadow over the detail |
| T28c | root-reveal-on-covering-select | deep-link a URL whose frontier list is unselected (no click) | the parent renders covered (32px peek) with the frontier in the flow beside it — no reveal, since no pointer is in the stack |
| T29 | title-each-list | render levels with `title` | each list shows its left-aligned title + a divider above the first row |
| T30 | mark-selection-without-bar, keep-connectors-attached-when-covered | select root + child in the covered stack | no gold bar; the root selected row has a leading dash; a gold elbow connects the selected parent row to the selected child row, staying attached when the parent is covered |
| T31 | keep-bar-for-standalone | render a standalone `TopicDetail` with a selection | the selected row shows the classic gold left-bar (no dash/connector) |
| T32 | allow-manual-cover | click a list's `»` cover toggle, then resize | the list stays covered (icon-strip peek) across the resize |
| T33 | default-to-auto-hide | render 3 levels, deepest selected, at 1440 | only the leaf-most list is disclosed; both parents peek even though there is room for them |
| T34 | offer-auto-hide-toggle | click the root list header's toggle | it flips to the "off" state and every list discloses; clicking again re-hides every parent |
| T35 | toggle-all-on-modifier-click | ⌘/Ctrl-click a `«` | EVERY list covers to its peek (not just the clicked one's parent); ⌘/Ctrl-click a `»` uncovers them all |
| T36 | keep-detail-at-minimum-while-lists-yield, shift-off-screen-by-whole-lists | narrow the container to just under (peeks + `minDetailWidth`) with a leaf selected | the detail sits at EXACTLY `minDetailWidth`; the leftmost list is `inert` at `left = −(its width)`; the shift equals that width exactly (not the overflow amount) |
| T37 | land-structure-changes-in-place | select a topic where the detail's left edge moves | the detail is at its final `left`/`width` on the FIRST frame after the click (no 300ms slide) |
| T38 | animate-width-changes | click a `«`/`»` where the detail's left edge moves | the detail's `left` is still mid-flight one frame later (it eases) |
| T39 | reverse-on-grow | with auto-hide OFF, narrow until lists hide/drill off, then widen back | the lists slide back on (specific → general) and re-disclose; with auto-hide ON they slide back on but stay hidden |
| T40 | exhaust-wide-before-narrow, keep-selection-across-modes | with a leaf selected, narrow the window past (`minDetailWidth` + one collapsed strip) | the frame switches to the navigation stack — the detail is the sole full-width pane — with the SAME selection; widening restores the covered columns unchanged |
| T41 | show-one-full-width-pane | narrow, workspace selected, feature not | the FEATURES list (the frontier) is the whole view at full width — not a landing pane telling you to pick one; the workspaces pane is `inert` + `aria-hidden` behind it, and no peek / cover toggle / auto-hide toggle is rendered |
| T42 | push-and-pop-like-a-navigation-controller | narrow, select a topic | the detail pane animates IN from the right edge (it is still mid-flight one frame after the click, not already in place) and the list behind it parallaxes |
| T43 | offer-back-in-narrow, guard-unsaved-on-exit | narrow, detail open, press Back | the deepest selected level is cleared, the detail animates OUT to the right and the list you chose from is the visible pane; a dirty leaf raises Save / Discard / Cancel first. The ROOT pane has no Back |
| T44 | exhaust-wide-before-narrow | load on an iPhone/Android phone UA at a viewport wide enough for the wide layout | the narrow layout is used anyway (a phone has no pointer for peeks / reveal / drag), and it stays narrow at every width |
| T45 | keep-view-state-across-a-selection | turn auto-hide OFF, then pick another row in the root list (a route change, so the subtree remounts) | the lists stay disclosed and the toggle stays off — the user's click does not silently reset the view they just arranged |
| T46 | keep-view-state-across-a-selection, close-branch-only-on-pointer-exit | hover a covered list to open its branch, then click a row inside it | the branch is still open after the remount (the pointer never left it), and closes only when the pointer leaves |
| T47 | keep-view-state-across-a-selection, default-select-a-level | on a level with a `defaultSelectedId`, clear the auto-selected row (Back, or re-click) | it STAYS cleared — the default does not re-fire on the remount the clear caused |
| T48 | grow-the-branch-walking-outward | hover the middle peek to open its branch, then move the pointer LEFT into the peek beside it | the shallower list joins the cascade as its new root; every list that was open stays open, pushed right |
| T49 | portal-modals-out-of-the-stack | narrow mode, nothing selected: open a level's `+` create dialog and type into it | the dialog is a child of `<body>` (not of the inert detail pane), the field accepts input, and Save enables |
| T50 | slide-panes-ease-in-out | narrow, in the real app (so the selection remounts the subtree): select a row, sampling the incoming pane's x each frame | it travels through intermediate positions from the right edge to 0 — not two samples (start, end) — and the offsets ease in and out (small deltas at both ends, largest in the middle). Back samples the same slide in reverse |
| T51 | slide-panes-ease-in-out, respect-reduced-motion | narrow, `prefers-reduced-motion: reduce`, select a row | **SUSPENDED** — reduced-motion is currently disabled by a standing product decision (see Design Decision **Reduced motion is suspended for this component**); the slide-panes-ease-in-out half on its own is covered by T50 |
| T52 | fill-the-pane | narrow, a list with fewer rows than the screen is tall | the list's background covers the full pane (its height equals the pane's); nothing behind it shows through under the last row, and no border runs down the screen edge |
| T53 | show-row-disclosure-chevron | narrow, a pane with an enabled row and a `disabled` row | the enabled row carries a trailing chevron, the disabled one does not; the same rows in the WIDE covered stack carry no chevron at all |
| T54 | apply-disclosure-toggles-immediately | hover a covered peek so the reveal opens, then (without moving the pointer out) click a header's `«`/`»`, and separately the root header's auto-hide toggle | the reveal DROPS on the click and the stack settles to the new pin / mode state immediately — the layout visibly changes on the click itself, not later when the pointer happens to leave |
| T55 | not-land-a-grid-at-an-unselected-frontier | a frontier list whose rows carry long unbreakable labels (full reverse-domain ids); no level sets anything but a boolean `overview` | the detail is the ONE centered nudge — no grid, no per-row card, and no second copy of any row's label anywhere in the pane |
| T56 | render-overview-help, show-select-nudge-at-unselected-frontier | a level with `itemNoun` + `overviewHelp` set, at its unselected frontier | the detail shows ONE centered card: `Select a <noun>` headline with the blurb under it; with an EMPTY list the blurb shows alone; with `overview: false` the nudge is suppressed and the host's `children` show |
| T57 | hold-the-detail-until-the-final-choice | cascade: a final choice's detail is showing; click a topic (top menu or submenu) that discloses another topic list | the submenu opens and the detail pane is UNCHANGED — still the previous detail, not the new topic's overview or a landing |
| T58 | hold-the-detail-until-the-final-choice | continue from T57: click a row that leads to no further topic list (the final choice) | the detail changes ONCE, straight to the final choice's detail — no intermediate overview/landing/blank ever showed during the walk |
| T59 | auto-collapse-menus-on-final-choice | cascade, auto-collapse ON: make a final choice and leave the pointer where it clicked | the open menus collapse inward (animated) on the click itself — before the pointer leaves the menu region — and the final choice's detail shows |
| T60 | auto-collapse-menus-on-final-choice | cascade, auto-collapse OFF: make a final choice with submenus open | only the detail swaps; every open menu stays exactly as arranged (no select collapses anything) |
| T61 | not-move-the-menus-on-an-intermediate-select | cascade, auto-collapse ON: click a row in a disclosed list whose select discloses a submenu (the frontier advances) | the clicked list does not move or re-cover — the submenu appears beside it at the disclosed advance; every list already on screen stays exactly where it was, with no pointer movement required to hold it |
| T62 | release-the-hold-when-the-gesture-ends | cascade: a detail is showing; re-click the selected root row (a clear), or the frontier menu's ✕, or a breadcrumb | the hold releases WITH the clear — the pane shows the real frontier state (overview / landing) as the navigation lands, on this surface and every one navigated to next; no stale pane follows the walk |
| T63 | hide-off-screen-progressively, exhaust-wide-before-narrow | every list collapsed, leaf selected, detail at `minDetailWidth`; shrink the window one strip-width at a time | exactly ONE more list slides off the left edge per step (never several at once); only after the LAST strip goes does the frame switch to the navigation stack |
| T64 | crossfade-detail-swaps | a detail is showing; select a different row (same level or deeper) | the outgoing pane dissolves into the incoming one over ~220ms — both halves fade, the pane itself does not move or resize — including when the selection is a route change that remounts the subtree |
| T65 | crossfade-detail-swaps | a surface's very first paint (nothing was showing), and separately a mount-time settle (levels registering) | the content simply appears — no fade-in, no intro — in both cases |
| T66 | log-layout-decisions-outside-production | on a non-production build, resize across a collapse, an off-screen step and the narrow flip | one `[htdv#…]` console line per decision (mode with width+floor, fit outcome, toggle, narrow push/pop); `__htdvLogDump()` returns the trace; a production build logs nothing |
| T67 | keep-selection-across-modes | a MERGED stack whose deepest levels are published from `children` (feature-internal state, not the URL), all levels selected; shrink past the narrow floor, then widen back | the frame goes narrow with the detail as the top pane and STAYS narrow (no bounce back to wide); widening restores the wide layout with every selection — including the children-published ones — exactly as it was |
...
- **Wide / narrow + the branch reveal (2026-07-11).** The frame owns ONE measured row (`useContainerWidth`
  ...
  same number and there is still a single disclosure controller. `const stripPx = disclosureStyle === "minimized" ? COLLAPSED_RAIL : COVERED_PEEK; const wideFloor = Math.max(minDetailPx(minDetailWidth) + stripPx, PHONE_FLOOR); const narrow = layoutMode === "narrow" || (layoutMode === "auto" && (phone || (containerW > 0 && containerW < wideFloor)))`; `usePhoneUserAgent` reads the UA AFTER mount
...
- **React / Web (TypeScript).** Enclosing frame:
  ...
  **Covered style:** a covered list's wrapper is `overflow-hidden` clipped to a 32px peek (its rows are
  ...
  `hoverId`) so the wipe-shut happens over the detail, and the close is
  ...
  **Unify-via-context.** Features publish their levels up rather than nesting their own
  ...
  boolean). Reduce-motion support exists in the global `@agenticdevelopertoolkit/themes` accessibility CSS (zeroes
  transition durations), but is currently SUSPENDED for both hierarchical views by a standing product
  decision — see Design Decision **Reduced motion is suspended for this component**. Hub home is gated — verify shared UI on the
...
- **SwiftUI:** Implement the wide layout with `NavigationSplitView` (macOS/iPadOS) and the narrow layout with `NavigationStack`, each level's `selectedId` bound to `@State`/`@Binding`. Render each level's rows as a `List` section; the covered style maps to a custom overlay view (not a system `Popover`) for the branch reveal, with positioning computed from row frame geometry (`GeometryReader` / `.overlayPreferenceValue`). The detail pane is a conditional `NavigationLink` destination (wide) or the top of the `NavigationStack` (narrow). Unsaved-work guarding is a custom back guard, not `@Environment(\.scenePhase)` (which reports foreground/background, never navigation intent): gate the pop with `.interactiveDismissDisabled(isDirty)` and present a `.confirmationDialog` (Save / Discard / Cancel) before it proceeds. Deep links reconstruct the `selectedId` hierarchy from URL path components.
- **Compose:** Structure the wide layout as a `Row` of side-by-side `LazyColumn`s — one per level, flat siblings, not nested — plus the detail `Composable` in the same `Row`, mirroring the source's flat-sibling-grid contract. Covered disclosure uses `AnimatedVisibility` with scrim overlays; minimized uses fixed-width columns that shrink to an icon strip. Selection state lives in a shared ViewModel; navigate via `NavController` for the narrow (single-pane) layout. Unsaved-work guards hook into `BackHandler` (Compose's back-press API) with a custom confirmation dialog.
...
- **WinUI 3:** Build the wide layout as `Grid` columns — one column per level (each a `ListView`) plus the detail column — not a hierarchical `NavigationView` (which models one selected destination, not a row of sibling lists). Bind each row's selection to update the next level's `ItemsSource` via each `ListView`'s `SelectionChanged` (cascading data binding). Covered disclosure uses `Popup` overlays for branch reveals with Canvas-based 32px peeking on the outer container. Minimized style uses a horizontal `StackPanel` with column-width bindings. The detail pane is a separate content region in a `SplitView`. Navigate via each row's `SelectionChanged` handler — not a hyperlink command binding, which is for inline text links, not list-row navigation. Unsaved changes trigger a `ContentDialog` before navigation away. Deep links parse URL path segments and recursively select each level in the hierarchy using data-binding selection commands.

## Design Decisions

- **Decision**: The frame renders the lists + the detail as flat sibling grid columns, not nested panes.
  **Rationale**: The rendered row is visually identical, but ONE ResizeObserver sees the whole row (correct
  general→specific collapse) and the leaf keeps a stable slot (no remount/flash on list-count change) — this
  is what makes the single-controller drill-down possible (separation-of-concerns, simplicity).
  **Approved**: pending
- **Decision**: Every list is a level of the single stack; the deepest pane is only ever a detail.
  **Rationale**: Dismantling the in-pane master/detail (list+editor in one pane) into a published list level
  + a leaf editor keeps the model uniform and deep-linkable, and removes a whole class of nested-observer
  bugs (principle-of-least-astonishment, dry).
  **Approved**: pending
- **Decision**: A single full-width breadcrumb + help spans every list; the feature publishes its levels up
  through the `WorkspaceChrome` context (rather than nesting its own frame), so the shell renders ONE merged
  frame.
  **Rationale**: The entity selector is a first-class list, not a popup — this recipe supersedes the earlier
  popup-driven pattern for workspace routes.
  **Approved**: pending
- **Decision**: `onSelect`/`onClear` are pure navigation; the package decides which fires (re-click →
  `onClear`) and never auto-selects.
  **Rationale**: Consumers write no toggle/resume logic — unselection and no-auto-select hold uniformly
  (dry).
  **Approved**: pending
- **Decision**: Off-screen drilling isn't a special small-screen mode bolted on — it is the same
  single-controller fit math, so phones simply start with every list drilled off and the detail full-width,
  Back walking up.
  **Rationale**: The manual disclosure toggle coexists (a collapsed list counts as its icon width, then
  hides), so one fit controller serves both wide and phone layouts (optimize-for-change).
  **Approved**: pending
- **Decision**: The create affordance (`onNew`) rides right-justified in the titled header rather than as a
  reserved leading row, so the first topic sits directly under the header.
  **Rationale**: Row alignment across lists comes from the uniform titled header (same height + divider),
  not a reserved-when-empty slot — every consumer built the same "New …" button, so the component owns one
  ...
  **Approved**: pending
- **Decision**: The editor's button bar is the top of its leaf pane; its dirty/save state is published as a
  `PaneExitGuard`.
  **Rationale**: This lets the package prompt Save/Discard/Cancel before any Back / up-nav discards work.
  **Approved**: pending
- **Decision**: A single keyed `site-config` store keeps help text out of components.
  **Rationale**: Consistent help copy across the platform (dry).
  **Approved**: pending
- **Decision**: The default `covered` style overlaps the lists like a stack of cards so the whole ancestry
  stays glanceable (a 32px peek) while the child + detail take the room — versus `minimized`, which slides
  parents fully off-screen.
  **Rationale**: Both share the one fit controller; covered is the better default on wide screens, and the
  same off-screen drill-down is the phone layout (principle-of-least-astonishment).
  **Approved**: pending
- **Decision**: A covered list shows only icons, so the pointer entering it lifts the ENTIRE real rail (full
  rows + header) above its neighbours rather than popping a per-row/per-header copy — one legible disclosure
  instead of many, disclosed only while the pointer is inside it.
  **Rationale**: Because it is the real rail (not a copy) there is no aria-current duplication and clicking
  a row is a pure select (never a toggle), keeping the covered interaction predictable (simplicity,
  principle-of-least-astonishment). The reveal is pointer-driven only, never focus-driven — see
  **reveal-all-covered-lists-on-hover**.
  **Approved**: pending
- **Decision**: Hovering a peek opens the whole BRANCH (the list + its children, chained) rather than
  lifting just that one list, because the branch is the unit the user is actually reading.
  **Rationale**: An earlier version lifted only the hovered list, leaving its children covered behind it —
  telling you what you were choosing between but not what choosing would show you. Opening the branch makes
  the close condition a property of the GROUP (the pointer leaving every member), not of one list — walking
  from a list into its own revealed child must not collapse what you are walking through
  (principle-of-least-astonishment). The branch itself no longer floats over the detail — see
  **push-detail-aside-on-reveal**, which supersedes the branch's original over-the-detail placement;
  only the covered peeks behind the branch stay overlapped.
  **Approved**: pending
- **Decision**: Once the wide layout's shrink sequence is exhausted — every list collapsed, the detail at
  its minimum, every collapsible list slid off-screen — the frame swaps the whole presentation for the
  platform-native one (`UINavigationController`) rather than shrinking the wide affordances further.
  **Rationale**: At that point peeks, cover toggles, the hover reveal and drag-resize are all pointer
  affordances spending room that no longer exists — and on a phone there is no pointer at all — so shrinking
  them further would be affordances masquerading as functionality. The same stack, selections,
  `onSelect`/`onClear` and unsaved guard continue underneath; nothing about a feature is phone-aware
  (native-controls, optimize-for-change). See **exhaust-wide-before-narrow** for the exact trigger.
  **Approved**: pending
- **Decision**: In the stack the selection is shown by a root dash + parent→child elbow connectors rather
  than a per-list left-bar.
  **Rationale**: The relationship between the selected rows reads at a glance across covered lists; the
  standalone primitive keeps its bar. The connector measurement and SVG are one shared
  `useSelectionConnectors` / `SelectionConnectorOverlay` used by both stacks (dry).
  **Approved**: pending
- **Decision**: `respect-reduced-motion` (the MUST requiring every animation to honor the user's
  reduce-motion preference) is currently SUSPENDED for both hierarchical views by a standing instruction from
  the block's owner — the `motion-reduce:` gates were removed so the two views cannot disagree once one flag
  picks between them.
  **Rationale**: This is a deliberate, temporary product decision (not a claim about anyone's OS
  configuration or a defect in the source); restore both views' gates together when the instruction is
  lifted. T18 and T51, which assert reduced-motion behavior, are marked suspended rather than deleted so
  they can be re-enabled the moment the gates return.
  **Approved**: pending
...
Keyboard/AT coverage rests on the source's `aria-current`, `aria-hidden`, `inert`, `aria-label` and live-region
(`role="status"`) usage throughout the rail, connector and narrow-pane code; reduced-motion is `partial`
because **respect-reduced-motion** is currently suspended by a standing product decision rather than
removed from the contract (see Design Decisions); deep-linking and native-controls-preference (the narrow
mode's platform-native navigation-controller equivalent) are both MUST requirements the source implements end
to end; and RTL fails because the layout is computed entirely in literal left/right terms (the covered peek's
leading/left cover control, the narrow slide's right-edge push/pop, `translateX` percentages) with no logical
start/end substitution anywhere in the source.

## Change History

| Version | Date | Author | Summary |
|---|---|---|---|
| 1.21.0 | 2026-09-22 | Mike Fullerton | Lint pass: renamed every requirement to subject-only kebab-case (no `must-`/`should-`/`may-` prefix) everywhere it's cited; reformatted Design Decisions into Decision/Rationale/Approved and added one for the suspended reduced-motion MUST; rebuilt Compliance as linked catalog checks (added accessibility/platform-compliance/internationalization, RTL failed); removed two stale narrative blockquotes and corrected several source-contradicting passages (narrow/wideFloor formula, covered-peek size 32px, branch-reveal wording, detail-action-bar wording); corrected wrong SwiftUI/Compose/WinUI 3 API claims in Platform Notes; completed the `TopicLevel`/`TopicDetailItem` interface sketches; fixed the update-url-on-select requirement and T3 to stop implying an auto-selected default; sorted Change History descending and moved the stray `headerSlot` props row into the interface sketch; renumbered T49-T53 and reconciled T18/T25/T26/T28a/T28c/T51 to current requirement names and values; added a reference-consumer note and replaced hardcoded example URLs/paths with placeholders. |
