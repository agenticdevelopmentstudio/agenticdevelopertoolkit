...
license: MIT
summary: Deep-linkable stack of collapsible topic lists under one breadcrumb, with hover-reveal, create affordance, crossfading detail pane, and narrow-mode navigation.
platforms:
- typescript
- web
tags:
...
The **package** decides which fires — a click on the already-selected row pops one
level: it calls the CHILD level's `onClear()` when that child currently has a
selection, and only the row's own `onClear()` once the clicked row is itself the
deepest selection; a click on any other row calls `onSelect(id)`. **Unselection is
uniform and package-owned**: consumers write no `prev === id ? null : id` toggle — the
pop-one-level-at-a-time behavior lives in the package's `railOnSelect`, shared by every
stack. The package **never auto-selects** — landing on a level with no selection shows the list with
...
### Disclosure — pins, then width pressure (the covering click floats, never snaps)

Absent a `«`/`»` pin, every topic list is DISCLOSED: there is no default that hides ancestors
ahead of the fit rules below, and there is no root-header toggle for one. (A leaf-first default —
hiding every list above the deepest rendered one — and a STATE toggle for it were designed and
shipped for a time, then parked before this version; see the Web platform note.)
...
Disclosure therefore has two layers, and **each may only ever HIDE more, never disclose**:

1. **pins** — the user's own `«`/`»` intent on one list,
2. **width pressure** — the fit rules below, which may take the room back from a list the user pinned
...
5. The **frontier** (a list with nothing selected yet) is never collapsed for width or slid off:
   the detail's minimum is fixed in every state, but the frontier is never counted among the
   lists that collapse or shift, and in the last-resort squeeze an unselected frontier's landing
   placeholder gives up its width before the list narrows, so the list you are choosing from
   always keeps its place. For an unselected frontier the wide layout ends only when
...
collapsed lists re-open — **unless the user pinned one shut**, because a pin is an intent layer
above the fit rules, not a consequence of narrowness.
...
The COVERING CLICK is different (v1.13.0): the reveal a select roots (see disclosure above) opens ONLY
...
member of it, or you **click a row**. Entering a *different* covered list already inside the open
group changes nothing — the reveal already opened every on-screen list from the first pointer
entry, so a later enter on another member is a no-op; it never re-roots there.
...
they are fully **interactive**: clicking a row behaves exactly like clicking that row at rest — the
same select/clear intent as **own-unselection** (re-clicking the already-selected row pops one level,
it does not no-op) — and the headers' `+`/`«` work in place. There is **no per-row or per-header
popover** — the branch is revealed at once.
...
Narrow mode is entered when the container is narrower than a fixed floor: the detail's minimum plus
ONE collapsed strip (32px covered peek / the minimized icon strip), floored at a flat 480px regardless
of what is selected — the floor is deliberately selection-independent, so losing a selection never by
itself flips the mode back to wide. It is ALSO entered — at any width — when the browser is a
**phone** (an iOS / Android phone user agent), which is why a phone starts and remains in narrow mode.
Tablets and desktops are decided by width alone — a narrow *window* on a big screen behaves exactly
like a phone, and a wide one doesn't.
...
| Alert & Dialog | agenticdevelopertoolkit://recipes/alert-and-dialog | The modal opened by a list's "new topic" button to create an item, AND the shared 2-action Discard/Stay unsaved-work prompt the package raises before a drill-down Back / breadcrumb-up discards a dirty leaf. | yes | New-item modal (returns the created id); the shared `UnsavedChangesAlert` (driven by `exitGuard`, never saves). |
...
- **resize-by-drag**: A topic list MUST be horizontally resizable by dragging its trailing border. In the `minimized` style this snaps at its extremes (**snap-undisclosed-when-narrow** / **snap-full-when-wide** below); in the default `covered` style there is no icon strip to snap to, so the drag instead resizes FREELY within a fixed readable range — clamped to a 160px minimum and a 640px maximum — and never snaps to undisclosed or back to a content width.
- **snap-undisclosed-when-narrow**: In the `minimized` style, if dragged narrower than 1/3 of the list's full rail width (240px, so an 80px threshold), a topic list MUST animate to undisclosed. This does not apply to the covered style — see **resize-by-drag**.
- **snap-full-when-wide**: In the `minimized` style, if dragged wider than the list's full rail width (240px), a topic list MUST clamp (back) to that width — a fixed rail width, not the list's own content size. This does not apply to the covered style — see **resize-by-drag**.
...
- **own-unselection**: A click on the already-selected row MUST pop one level: if the clicked level's CHILD currently has a selection, the click MUST clear that child (`child.onClear()`) rather than the clicked row's own level; only once the clicked row is itself the deepest selection does the click clear that level (`onClear()`). A click on any other row MUST select it (`onSelect`). Consumers MUST NOT write toggle logic — the pop-one-level-at-a-time behavior lives in the package's shared `railOnSelect`, used by every stack (minimized, covered, narrow).
...
- **guard-unsaved-on-exit**: If the leaf editor is dirty (`exitGuard.isDirty()`), any action that would clear OR replace the open detail — Back, re-click-deselect, breadcrumb up-nav, selecting a shallower row, and selecting a **different row in the deepest selected level itself** (a sibling swap that unmounts the open leaf) — MUST first open the shared **2-action Discard / Stay** `UnsavedChangesAlert` — Discard → proceed; Stay → keep editing. It never saves: `exitGuard` (`PaneExitGuard`) exposes only `isDirty()`, with no `save()` for the frame to call. Only a forward drill-down into a not-yet-selected level (no open detail to lose) is unguarded; equivalently, `railOnSelect` guards whenever the target level already has a selection (`level.selectedId != null`).
...
- **reverse-on-grow**: Widening MUST re-run the same rules on the wider container, so on a larger device leaving narrow mode discloses the lists in reverse order — off-screen lists slide back on (specific → general) and hidden lists re-disclose — but a list MUST re-disclose ONLY when the user has not pinned it shut.
...
### Disclosure toggles — pins and modifier-click

- **disclose-absent-a-pin**: Absent a `«`/`»` pin, a topic list MUST be disclosed: there is no default that hides a parent list ahead of the fit rules below, and the root list's header renders no toggle for one. (A leaf-first `autoHideTopics` default and a root-header toggle for it were designed and shipped for a time, then parked before this version — see the Web platform note; neither ships today.)
- **layer-hide-intent**: Disclosure MUST resolve as two layers where each may only ever HIDE more, never disclose: a user's `«`/`»` pin → width pressure (which MAY take the room back from a list the user pinned open, but MUST NOT disclose one they pinned shut).
...
- **cover-automatically**: A list MUST be covered automatically when there is not room to show it in full alongside the child lists and the detail at its minimum; the **frontier** choosing list (nothing selected yet) MUST NOT be covered, and MUST NOT be slid off-screen by the detail's minimum-width shift — the detail's minimum is the same in every state, the frontier is simply excluded from the lists that collapse or shift, and in the last-resort squeeze an unselected frontier's landing placeholder yields its width before the choosing list narrows.
...
- **keep-branch-open-while-inside-it**: The revealed branch MUST stay open while the pointer is inside ANY of its members — moving from the hovered list into one of its revealed children MUST NOT collapse it. It MUST close (animate back to the previous state) only once the pointer has left every member of the branch. Entering a DIFFERENT covered list that is already a member of the open group MUST change nothing (the whole-stack reveal already opened it); it MUST NOT re-root or narrow the group.
...
- **pure-select-from-reveal**: Clicking a row in a revealed list MUST behave exactly like clicking that row at rest — the same `onSelect`/`onClear` intent as **own-unselection** — removing the deeper lists and showing the chosen item's detail. Re-clicking the already-selected row is NOT a no-op here: it pops one level down, same as anywhere else in the stack.
...
- **exhaust-wide-before-narrow** (v1.17.0; supersedes switch-to-narrow-when-only-a-detail-fits): The frame MUST switch to the NARROW layout only once the wide layout's whole shrink sequence is exhausted — every list collapsed, the detail at `minDetailWidth`, every collapsible list slid off-screen. Concretely: narrow begins when the container is narrower than `minDetailWidth` plus ONE collapsed strip (32px covered peek / the minimized icon strip), floored at a flat 480px regardless of selection — the floor MUST NOT vary with the selection (an unselected frontier's "no detail minimum" exemption belongs to the fit math, never to a lower narrow floor). The frame MUST ALSO be narrow — at any width — when the browser is a phone (an iOS / Android phone user agent), so a phone starts and remains in narrow mode; the 480px floor itself guarantees the frame never drops narrower than phone width even when `minDetailWidth` is small. Tablets and desktops are decided by width alone, so a narrow WINDOW behaves exactly like a phone and a wide one does not. The mode MUST be resolved before paint (no flash of the wide layout), and `layoutMode="wide" | "narrow"` MUST force one. The superseded rule (one full topic list + the detail minimum) sat ABOVE the width where off-screen hiding begins, so the mode flip hid every list at once and the progressive drill-down never ran.
- **show-one-full-width-pane**: In narrow mode each topic list and the detail MUST be a FULL-WIDTH pane, and exactly ONE of them is visible: the frontier list while it is still being chosen from, the detail once every level is selected. A landing placeholder MUST NOT be shown in place of the list being chosen from. Peeks, cover toggles, the hover reveal and drag-resize MUST NOT be rendered — they spend room that does not exist.
...
| T13 | resize-by-drag, snap-undisclosed-when-narrow | in the `minimized` style, drag a rail below 1/3 of its full width (80px) | rail animates to undisclosed; the default `covered` style clamps the same drag at a fixed 160px minimum instead and never undiscloses |
| T14 | resize-by-drag, snap-full-when-wide | in the `minimized` style, drag a rail wider than its full rail width (240px) | rail clamps back to that fixed width; the default `covered` style clamps the same drag at a fixed 640px maximum instead, with no snap |
...
| T19 | own-unselection | click the already-selected entity row while its child (features) has a selection | the CHILD level (features) clears, not the clicked row's own level — a second click on the (now leaf) entity row is needed to clear it |
...
| T22 | guard-unsaved-on-exit | edit the leaf editor (dirty), then press Back | the 2-action Discard/Stay `UnsavedChangesAlert` opens; Discard proceeds, Stay keeps editing — there is no Save leg, the alert never persists |
...
| T26b | keep-branch-open-while-inside-it | hover the covered root, then move the pointer into a DIFFERENT covered list already inside the open group | nothing changes: every on-screen list opened on the first pointer entry, so the new list was already open and stays open — no re-root, no list returns to its peek |
| T26c | float-branch-as-an-opaque-card | hover a covered list in the MIDDLE of the stack, with no lists yet drilled off-screen | entering it opens EVERY on-screen list, including the ones to its left, so there is no leftover peek to shadow over — only the TRAILING shadow (over the detail) shows; no member draws a leading shadow |
...
| T27 | pure-select-from-reveal | click a row in a revealed list | that row becomes selected, deeper lists clear, its detail shows; clicking the already-selected row pops one level (same as **own-unselection**), it does not no-op |
...
| T28a | root-reveal-on-covering-select | width pressure covers the ancestors, only the deepest list disclosed; click a row whose selection pushes a new (unselected) child list | the clicked list stays open in place (covered in layout, held by the reveal) and the new list slides out beside it OVER the detail; moving the pointer off the branch settles it — the parent to its 32px peek, the new list into the flow |
| T28b | root-reveal-on-covering-select | same narrow layout; click a row in the LAST level (the selection completes the path) | no reveal engages: the stack lays out at rest (parents peek, the clicked list disclosed) with no lift and no floating-card shadow over the detail |
...
| T33 | disclose-absent-a-pin | render 3 levels, deepest selected, at 1440, no pins set | all three lists are disclosed at once — there is no default that hides a parent ahead of the fit rules, and the root header renders no toggle |
| T34 | layer-hide-intent | click one list's own `«` (no modifier) | only that list covers to its 32px peek; its siblings are unaffected; clicking its `»` re-discloses it alone |
...
| T39 | reverse-on-grow | narrow until lists hide/drill off, then widen back | the lists slide back on (specific → general) and re-disclose; a list the user pinned shut slides back on but stays covered |
...
| T41 | show-one-full-width-pane | narrow, workspace selected, feature not | the FEATURES list (the frontier) is the whole view at full width — not a landing pane telling you to pick one; the workspaces pane is `inert` + `aria-hidden` behind it, and no peek / cover toggle is rendered |
...
| T43 | offer-back-in-narrow, guard-unsaved-on-exit | narrow, detail open, press Back | the deepest selected level is cleared, the detail animates OUT to the right and the list you chose from is the visible pane; a dirty leaf raises the 2-action Discard/Stay alert first (no Save leg). The ROOT pane has no Back |
...
| T45 | keep-view-state-across-a-selection | pin a list shut, then pick another row in the root list (a route change, so the subtree remounts) | the pinned list stays covered — the user's click does not silently reset the view they just arranged |
...
| T54 | apply-disclosure-toggles-immediately | hover a covered peek so the reveal opens, then (without moving the pointer out) click a header's `«`/`»` | the reveal DROPS on the click and the stack settles to the new pin state immediately — the layout visibly changes on the click itself, not later when the pointer happens to leave |
...
- **Wide / narrow + the branch reveal (2026-07-11).** The frame owns ONE measured row (`useContainerWidth`
  ...
  **The branch reveal** in `CoveredStack`: entering any covered (peeking) list sets `hoverId` to that
  list and `hoverAll` to true, which makes the group every ON-SCREEN list (from the leftmost visible
  one, not just the hovered list and its children) — revealed members take `railWidth` and chain from
  the leftmost on-screen list's own left edge, lifted to `z 50 + i` above the detail. Entering a
  DIFFERENT on-screen list once the group is open is a no-op (`inGroup(i)` is already true, so the
  handler returns before touching `hoverId`/`hoverAll`) — the group never re-roots or narrows.
  Closing is a
...
- **Pins + the fit rules (2026-07-11; a since-parked `autoHide` layer is gone).** The frame OWNS the
  disclosure intent — `pins: Record<levelId, boolean>` — and passes it to whichever stack renders, so
  the two layouts share one contract and differ only in how a hidden list is DRAWN (a 32px peek vs an
  icon strip). Each stack resolves `pinned(i) = pins[id] ?? false` (no auto-hide fallback), then adds
  width pressure on top — pressure only ever ADDS a hide, so a pin shut is never disclosed.
...
- **React / Web (TypeScript).** Enclosing frame:
  ...
  "minimized"` (default `covered`), `layoutMode?: "auto" |
  ...
  `wideFloor`: `minDetailWidth` + one strip, floored at `PHONE_FLOOR` (480px); this does NOT vary with
  the selection — an unselected frontier's own no-minimum landing is an exemption that belongs to the
  fit math, not to a lower narrow floor — or a phone UA), else a
  ...
  **Covered style:** a covered list's wrapper is `overflow-hidden` clipped to a 32px peek (its rows are
  always FULL, so only the leading icon shows — nothing of the label). Hovering it opens EVERY
  on-screen list — parents and children alike, not just the hovered list and what's below it — via an
  animated width **wipe** + a `left` chain (`CoveredStack` sets `hoverAll` true and chains members from
  the leftmost on-screen list's own left edge, not from the hovered list), lifted above
  ...
  children — no portal copy, pointer-driven only (no focus reveal); entering a different on-screen list
  once the group is open never re-roots or narrows it. **Selection markers:** the shared `useSelectionConnectors` hook + `SelectionConnectorOverlay`
  ...
  unselected), the **Back** button (leftmost-visible pane), and the **2-action
  Discard/Stay** `UnsavedChangesAlert` driven by `exitGuard` (`attemptExit`) — it never saves;
  `PaneExitGuard` exposes only `isDirty()`.
...
- **Decision**: The editor's button bar is the top of its leaf pane; its dirty/save state is published as a
  `PaneExitGuard`.
  **Rationale**: This lets the package raise the shared 2-action Discard/Stay `UnsavedChangesAlert` before any Back / up-nav discards work; the guard exposes only `isDirty()`, so saving remains the leaf editor's own affordance.
  **Approved**: pending
...
- **Decision**: A covered list shows only icons, so the pointer entering it lifts the ENTIRE real rail (full
  ...
  **Rationale**: Because it is the real rail (not a copy) there is no aria-current duplication and clicking
  a row behaves exactly like clicking it at rest (**own-unselection**), keeping the covered interaction predictable (simplicity,
  ...
  **Approved**: pending
- **Decision**: Hovering a peek opens EVERY on-screen list — parents and children alike, not just the
  hovered list's own children — rather than lifting just that one list, because the whole ancestry is
  the unit the user is actually reading.
  **Rationale**: An earlier version lifted only the hovered list, leaving its children covered behind it —
  telling you what you were choosing between but not what choosing would show you. A later version opened
  only the hovered list's own branch (itself and its children), which still left the hovered list's own
  PARENTS covered — walking the stack leftwards was a list-by-list re-rooting exercise. Opening every
  on-screen list from one hover answers the whole question at once, and makes the close condition a
  property of the GROUP (the pointer leaving every member), not of one list — walking from a list into
  another already-open member must not collapse what you are walking through
  (principle-of-least-astonishment), and entering a different on-screen list never re-roots or narrows
  the group. The branch itself no longer floats over the detail — see
...
start/end substitution anywhere in the source. `separation-of-concerns` is partial because, although selection
and layout logic is broken out into dedicated hooks and pure functions (`railOnSelect`, `structureSignature`,
`useRailFit`, `useSelectionConnectors`), the cover/pressure calculations that decide which lists are disclosed
(`CoveredStack`, `MinimizedStack`, `NarrowStack`) are computed inline in those components' render bodies rather
than extracted alongside the rest of the logic; `unit-test-coverage` is passed on the nine dedicated test files
(`hierarchicalTopicDetail`, `cascadeRules`, `cascadeInteraction`, `railAutoFit`, `detailSwapContainment`,
`htdvExitGuard`, `breadcrumbLeadingSeparator`, `detailPaneMarker`, `topicDetailCheckable`), which render the real
`HierarchicalTopicDetail` and assert its cascade, exit-guard, rail-fit, and breadcrumb behaviors directly.

## Change History

| Version | Date | Author | Summary |
|---|---|---|---|
| 1.21.1 | 2026-09-25 | Mike Fullerton | Corrected disclosure/pin, unsaved-exit alert, wideFloor, own-unselection, hover-reveal, and minimized-style drag-snap sections; frontier passages now match the fixed-in-every-state detail minimum; shortened frontmatter summary. Added best-practices compliance rows (separation-of-concerns: partial, unit-test-coverage: passed). |
| 1.21.0 | 2026-09-22 | Mike Fullerton | Lint pass: renamed every requirement to subject-only kebab-case (no `must-`/`should-`/`may-` prefix) everywhere it's cited in this recipe (on-main Change History rows below 1.20.0 are unchanged, per the immutability rule — they still use the old prefixed names, and so does existing source test code); reformatted Design Decisions into Decision/Rationale/Approved and added one for the suspended reduced-motion MUST; rebuilt Compliance as linked catalog checks (added accessibility/platform-compliance/internationalization, RTL failed); removed two stale narrative blockquotes and corrected several source-contradicting passages (narrow/wideFloor formula, covered-peek size 32px, branch-reveal wording, detail-action-bar wording); corrected wrong SwiftUI/Compose/WinUI 3 API claims in Platform Notes; completed the `TopicLevel`/`TopicDetailItem` interface sketches; fixed the update-url-on-select requirement and T3 to stop implying an auto-selected default; sorted Change History descending and moved the stray `headerSlot` props row into the interface sketch; renumbered T49-T53 and reconciled T18/T25/T26/T28a/T28c/T51 to current requirement names and values; added a reference-consumer note and replaced hardcoded example URLs/paths with placeholders. |
