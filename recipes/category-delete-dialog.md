...
- **disables-actions-while-busy**: When `busy` is `true`, the component MUST replace both the "Delete" and "Cancel" buttons with a loading indicator and MUST block every dismissal path (Escape, backdrop click, the close affordance) until `busy` returns to `false`.
...
- **calls-oncancel-on-backdrop-dismissal**: When `busy` is `false`, dismissing the dialog via a backdrop click or the close (`×`) affordance MUST call `onCancel()`, the same as clicking "Cancel".
...
| category-delete-009 | disables-actions-while-busy | `busy={true}` | Delete and Cancel buttons are replaced by a loading indicator; Escape, backdrop click, and the close affordance no longer call `onConfirm`/`onCancel` |
...
| category-delete-019 | calls-oncancel-on-backdrop-dismissal | `busy={false}`, user clicks backdrop | `onCancel()` callback is invoked |
...
- **Busy state blocks Cancel too**: While `busy={true}`, there is no Cancel button to click — both actions are replaced by a loading indicator, and every dismissal path is blocked. `onCancel` MUST NOT be invoked by any path while `busy` is `true`. See **disables-actions-while-busy** (vector category-delete-009).
...
| `onCancel` | `() => void` | — | Callback invoked when the user clicks "Cancel", clicks the backdrop, or uses the close affordance while not busy. REQUIRED. |
