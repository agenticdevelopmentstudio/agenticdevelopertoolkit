...
- **skip-disabled**: A `disabled` item MUST be dimmed and non-interactive (no `onSelect`, no delete button, no hover affordance).
...
- **render-divider-after**: An item with `dividerAfter=true` MUST render a separator line after it (visible in expanded list only, not in collapsed/covered icon strips).
...
- **respond-mobile**: At viewport width ≤768px the rail MUST become a horizontal wrap below the detail pane with a bottom selection bar instead of a left bar.
...
| T16 | render-divider-after | `{dividerAfter: true}` | separator line below the item (expanded list only) |
| T17 | render-spacer-after | `{spacerAfter: true}` at position 2 in 5-item list | items 3–5 sit at the rail's bottom edge |
| T18 | respond-mobile | viewport 375px width | rail wraps horizontally below pane; rows have bottom bar (`border-b`) |
...
| 1.4.0 | 2026-07-14 | Mike Fullerton | `inlineSublabel` on `TopicDetailItem`: render `sublabel` on the label's line (dim, label-priority truncation) for dense single-line entity rows (sites / groups / platforms / users). New requirement `render-inline-sublabel`. |
...
| 1.2.0 | 2026-07-03 | Mike Fullerton | `spacerAfter` on `TopicDetailItem`: a flexible spacer after the item pins the following items to the rail's bottom edge (e.g. a bottom Settings), in the full rail and the collapsed/covered icon strips alike. New requirement `render-spacer-after`. |
