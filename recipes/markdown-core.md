...
- **byte-exact-round-trip**: `FrontmatterSplit`'s `prefix`, `block`, and `body` MUST concatenate back to the original input byte-for-byte, whether or not the input has a frontmatter block.
...
- **stale-response-suppression**: On success or error, `useMarkdownDocument` MUST drop the result when the controller was aborted for any reason other than the timeout reason (an unmount or an `id` change), but MUST still surface a timeout-then-late-response as the timeout error rather than silently discarding it.
...
- **real-yaml-parser-for-frontmatter**: `frontmatterOf(content)` MUST parse the frontmatter block with the `yaml` package's `parseYaml(..., { logLevel: 'silent' })` rather than a line regex, and MUST return `null` when the parsed result is not a plain object (including arrays and parse failures).
...
- **write-path-never-touches-name**: `setFrontmatterTitle` MUST NOT read, write, or delete a frontmatter `name` key under any circumstance.
...
| mdc-006 | frontmatter-block-detection, byte-exact-round-trip | 8 samples incl. CRLF, an unclosed block, a block not at position 0 | `prefix + block + body` reconstructs the original exactly; non-frontmatter content splits to no block | FrontmatterTests.swift "prefix and body always recombine into the original, byte for byte", "a block that is not at the very start is not frontmatter", "an unterminated block is not frontmatter" |
...
- A fetch aborted for the timeout reason that nonetheless resolves afterward MUST still be surfaced as the timeout error, not silently dropped the way an unmount/id-change abort is **(concurrent access)** — stale-response-suppression.
