...
- **disabled-state** (Wrapper): The component MUST accept a `disabled` prop that prevents user interaction when true (`Checkbox.tsx`, `disabled` prop; `disabled={disabled}` on the input).
