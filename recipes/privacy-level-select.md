...
- **initialize-null-cursor-to-first-option**: If the keyboard cursor is still null (the popover has never been opened in this component instance) when the user presses ArrowUp or ArrowDown, Component MUST move the cursor to the first navigable option (`only-me`) regardless of arrow direction. This path exists because the listbox surface stays in the DOM while closed (see **hide-on-close-visibility** on the Popover this component composes: agenticdevelopertoolkit://recipes/popover#requirements/hide-on-close-visibility), so a keyboard user can reach it and press an arrow key before ever opening the popover through the trigger.
...
| privacy-019 | initialize-null-cursor-to-first-option | Cursor never set (null); user tabs to the listbox surface while the popover is still closed and presses ArrowDown | Cursor moves to first option ("only-me") without error; the same happens if ArrowUp is pressed instead |
...
- **Keyboard navigation null cursor recovery**: If cursor is null and user presses ArrowDown, the `move` function initializes to index 0 (first option). If user presses ArrowUp with null cursor, it also initializes to index 0, not the last option — see **initialize-null-cursor-to-first-option**.
...
- **Decision**: When the keyboard cursor is null, both ArrowDown and ArrowUp move it to the first option (`only-me`) — there is no wrap-to-last-option behavior.
  **Rationale**: This is the implemented behavior. Under **reset-cursor-to-value-on-open**, the cursor is set to `value` as soon as the popover opens, so this null-cursor path can only run if the user reaches the listbox surface directly by keyboard before ever opening the popover — see **initialize-null-cursor-to-first-option**.
  **Approved**: pending
